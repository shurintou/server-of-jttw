...
module.exports = async function (wss) {
    ...
    try {
        const clearHandlerTimer = setInterval(async function checkConnections() {
            ...
            const sessionKeys = await redis.keys(conf.redisCache.sessionPrefix + '*')
            if (sessionKeys.length === 0) {
                ...
                if (gameRoomKeys.length > 0) {
                    ...
                    gameRoomKeys.forEach(gameRoomKey => { redis.del(gameRoomKey) })
                }
                const playerKeys = await redis.keys(conf.redisCache.playerPrefix + '*')
                if (playerKeys.length > 0) {
                    logger.warn('clearHandler cleared all players.')
                    playerKeys.forEach(playerKey => { redis.del(playerKey) })
                }
                return
            }
            const sessionList = await redis.mGet(sessionKeys)
            /** @type {RedisCacheWebsocketInfo[]} */
            let sessions = []
            sessionList.forEach(item => { sessions.push(JSON.parse(item)) })
            for (let i = 0; i < sessions.length; i++) {
                if (!sessions[i]) continue
                const ttl = await redis.ttl(conf.redisCache.sessionPrefix + sessions[i].sessionID)
            ...
            gameRoomList.forEach(async item => {
                ...
                if (stillHasPlayer) {
                    ...
                    redis.set(conf.redisCache.gameRoomPrefix + gameRoom.id, JSON.stringify(gameRoom))
                    const gameRoomKeys = await redis.keys(conf.redisCache.gameRoomPrefix + '*')
                    if (gameRoomKeys.length === 0) {
                ...
                await redis.del(conf.redisCache.gameRoomPrefix + gameRoom.id)
                const gameRoomKeys = await redis.keys(conf.redisCache.gameRoomPrefix + '*')
                if (gameRoomKeys.length === 0) {
                    wss.clients.forEach(function each(client) {
                        if (client.readyState === WebSocket.OPEN) {
