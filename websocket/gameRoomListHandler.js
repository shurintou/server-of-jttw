...
module.exports = async function (data, wss, ws) {
    ...
    try {
        ...
        /* id是0则直接获取现在的游戏房间列表 */
        if (data.id === 0) {
            const gameRoomKeys = await redis.keys(gameRoomKey)
        ...
        /* id为NaN是新建的房间，需要分配一个id */
            ...
            await redis.set(conf.redisCache.gameRoomPrefix + freeIndex, JSON.stringify(newRoomData))
            ...
            if (gameRoomLength > 1) {
                ...
                if (duplicateOwner) {
                    await redis.del(conf.redisCache.gameRoomPrefix + freeIndex)
                    gameRoomList.pop()
            ...
            /* 小于0，某玩家离开了房间 */
            const roomRes = await redis.get(roomId)
            ...
            if (stillHasPlayer) {
                ...
                if (room.owner === deleteId) {
                    room.owner = remainId
                    const playerList = await redis.get(conf.redisCache.playerPrefix + remainId)
                    wss.clients.forEach(function each(client) {
        ...
        /* 大于0，属于对某个房间的操作 */
            ...
            /* 换位置 */
                ...
                const roomRes = redis.get(roomId)
                if (roomRes === null) { return logger.error('gameRoom:' + roomId + errors.CACHE_DOES_NOT_EXIST) }
                /** @type {RedisCacheRoomInfo} */
                const room = JSON.parse(roomRes)
                /** @type {GamePlayerSeatIndex} */
