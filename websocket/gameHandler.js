    ...
    try {
        ...
        if (data.action === 'initialize') {
            await redis.executeIsolated(async isolatedRedis => {
                await isolatedRedis.watch(gameKey, gameRoomKey)
                const gameRes = await redis.get(gameKey)
                if (gameRes !== null) {
                    return
                }
                const gameRoomRes = await redis.get(gameRoomKey)
                const playerKeys = await redis.keys(conf.redisCache.playerPrefix + '*')
                const playerListRes = await redis.mGet(playerKeys)
                /** @type {RedisCacheRoomInfo} */
                const gameRoom = JSON.parse(gameRoomRes) //游戏房间
                /** @type {string[]} */
                const redisMSetStr = [] //mSet批量改变玩家游戏状态的redis语句
                ...
                await redis.multi()
                    ...
                    .exec()
                const newPlayerKeys = await redis.keys(conf.redisCache.playerPrefix + '*')
                const newPlayerList = await redis.mGet(newPlayerKeys)
                wss.clients.forEach(function each(client) {
                    if (client.readyState === WebSocket.OPEN) {
                        client.send(JSON.stringify({ type: 'playerList', data: newPlayerList }))
                    }
                })
                const newGameRoomKeys = await redis.keys(conf.redisCache.gameRoomPrefix + '*')
                const newGameRoomList = await await redis.mGet(newGameRoomKeys)
                wss.clients.forEach(function each(client) {
                    if (client.readyState === WebSocket.OPEN) {
...
async function deleteGame(game, wss, losePlayer, winPlayer) {
    ...
    try {
        ...
        await redis.set(gameRoomKey, JSON.stringify(gameRoom))
        const gameRoomKeys = await redis.keys(conf.redisCache.gameRoomPrefix + '*')
        const gameRoomList = await redis.mGet(gameRoomKeys)
        wss.clients.forEach(function each(client) {
            if (client.readyState === WebSocket.OPEN) {
        ...
        const changePlayerListRes = await redis.mGet(changePlayerList)
...
async function calRecord(player, playerInstance, averageCard, losePlayer, winPlayer, playerNum) {
    ...
    try {
        ...
        const res = await redis.exists(conf.redisCache.playerRecordPrefix + playerInstance.id)
